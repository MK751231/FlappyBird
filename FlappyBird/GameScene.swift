...
class GameScene: SKScene, SKPhysicsContactDelegate /* 追加 */ {
    ...
    // Audio
    let audioMissNode = SKAudioNode(fileNamed: "miss.mp3")
    let audioGetNode = SKAudioNode(fileNamed: "get.mp3")
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        if (contact.bodyA.categoryBitMask & scoreCategory) == scoreCategory || (contact.bodyB.categoryBitMask & scoreCategory) == scoreCategory {
            ...
            if itemScore > itemBestScore {
                ...
                itemBestScoreLabelNode.text = "BestItemScore:\(itemBestScore)"
    ...
    override func didMove(to view: SKView) {
        ...
        // SE setup
        audioMissNode.autoplayLooped = false
        self.addChild(audioMissNode)
        audioGetNode.autoplayLooped = false
        self.addChild(audioGetNode)
    ...
    func setupWall() {
        ...
        // アイテムの画像を読み込む
        let itemApple = SKTexture(imageNamed: "apple")
        itemApple.filteringMode = .linear
        // 移動する距離を計算
        ...
        let createWallAnimation = SKAction.run({
            ...
            // アイテムを作成
            let item_random_y = CGFloat.random(in: -40...80)
            let apple = SKSpriteNode(texture: itemApple)
            apple.position = CGPoint(x: 0, y: under_wall_y + itemApple.size().height + slit_length + random_y_range + item_random_y)
            // スプライトに物理演算を設定する
            apple.physicsBody = SKPhysicsBody(rectangleOf: itemApple.size())
            apple.physicsBody?.categoryBitMask = self.itemCategory    // ←追加
            apple.physicsBody?.contactTestBitMask = self.birdCategory
            // 衝突の時に動かないように設定する
            apple.physicsBody?.isDynamic = false    // ←追加
            wall.addChild(apple)
